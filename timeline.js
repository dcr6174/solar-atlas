...
export const TOTAL_DAYS=(MAX_DATE-MIN_DATE)/DAY;
...
// An integer slider step represents one exact UTC calendar day.
export function sliderToTime(position){return MIN_DATE+Math.max(0,Math.min(TOTAL_DAYS,Math.round(Number(position))))*DAY;}
export function timeToSlider(date){return Math.max(0,Math.min(TOTAL_DAYS,Math.floor((date-MIN_DATE)/DAY)));}
