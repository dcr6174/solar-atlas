...
try{
 ...
 await page.locator('#fullscreen').click();if(await page.locator('#play-state').textContent()!=='PLAYING')throw Error('Existing playback stopped on exit');await page.locator('#play').click();await page.locator('#speed').selectOption('10');
 ...
 if(!await small.evaluate(()=>document.documentElement.classList.contains('cinema'))||await small.locator('#body-list').isVisible()||await small.locator('.timeline').isVisible()||await small.locator('#play-state').textContent()!=='PLAYING')throw Error('Mobile cinema did not hide controls and play');
 await small.locator('#fullscreen').click();if(!await small.locator('#body-list').isVisible()||!await small.locator('.timeline').isVisible()||await small.locator('#play-state').textContent()!=='PAUSED')throw Error('Mobile interface did not return');
